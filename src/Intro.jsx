...
const Intro = () => {
  ...
  return (
  <>
  <div className=" flex justify-center p-1 flex-col items-center">
  ...
  <WiSunrise size={40} /> Sunrise - {new Date(Data.sys.sunrise * 1000).toLocaleTimeString()}
   <WiSunset  size={40} /> Sunset - {new Date(Data.sys.sunset * 1000).toLocaleTimeString()}
  </div>
  <div className="flex items-center text-sm md:text-xl gap-2 justify-center md:gap-4">
  <BsFillEyeFill size={32} /> Visibility - {Data.visibility/ 1000} km
   <WiStrongWind  size={32} /> Wind Speed - {Data.wind.speed} km/hr
  </div>
  <div className="flex items-center text-sm md:text-xl gap-2 justify-center md:gap-4">
  <AiOutlineArrowUp size={35} /> Maximum - { Math.floor(Data.main.temp_max / 10)} &deg;C
   <AiOutlineArrowDown  size={35} /> Minimum - {Math.floor(Data.main.temp_min / 10)} &deg;C
  </div>
  <div className="flex items-center text-sm mb-4 md:text-xl gap-2 justify-center md:gap-4">
  <BsDropletFill size={30} /> Humidity - {Data.main.humidity} % 
   <AiFillCloud  size={35} /> Cloud - {Data.clouds.all} %
  </div>
  </div>
